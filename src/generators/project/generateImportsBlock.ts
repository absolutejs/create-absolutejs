...
export const generateImportsBlock = ({
  ...
  const pushHandler = (cond: boolean, name: string) =>
    cond && rawImports.push(`import { ${name} } from '@absolutejs/absolute'`)
  ...
  const dialectImports = {
    ...
    planetscale: ["import { drizzle } from 'drizzle-orm/planetscale-serverless'"],
  ...
  if ((orm ?? 'none') === 'none' && databaseEngine === 'sqlite')
    rawImports.push(
      ...(databaseHost === 'turso'
        ? [`import { createClient } from '@libsql/client'`, `import { getEnv } from '@absolutejs/absolute'`]
        : [`import { Database } from 'bun:sqlite'`])
    )
