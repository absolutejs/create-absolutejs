...
const POSTGRES_SQL_ADAPTER_CONTENT = `import type { Pool, QueryResultRow } from 'pg'
...
export const ensurePostgresSqlAdapter = (backendDirectory: string) => {
	const databaseUtilitiesDirectory = join(backendDirectory, 'database');
	const adapterPath = join(databaseUtilitiesDirectory, 'createPgSql.ts');

	mkdirSync(databaseUtilitiesDirectory, { recursive: true });
	writeFileSync(adapterPath, POSTGRES_SQL_ADAPTER_CONTENT, 'utf-8');
};
