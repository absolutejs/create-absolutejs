...
const scaffoldAndTestMysql = async (
  config: TestMatrixEntry
) => {
...
const runSequentially = async (
  ...
  handler: (config: TestMatrixEntry, index: number) => Promise<MysqlTestResult>
) =>
  configs.reduce<Promise<MysqlTestResult[]>>(
...
const printSummary = (results: MysqlTestResult[]) => {
  ...
  failedResults.forEach((result) => {
    ...
    console.log(
      `\n- MySQL + ${failureConfig.databaseHost} + ${failureConfig.orm} + ${failureConfig.authProvider}`
    );
...
export const runMysqlTests = async (
  ...
  console.log(
    `Testing ${configsToTest.length} MySQL configurations (${matrixEntries.length} total in matrix)...\n`
  );
  ...
  const results = await runSequentially(configsToTest, async (config, index) => {
    ...
    console.log(
      `[${index + 1}/${configsToTest.length}] Testing MySQL + ${config.orm} + ${authLabel} + ${hostLabel}...`
    );

    const outcome = await scaffoldAndTestMysql(config);
...
if (import.meta.main) {
  ...
  runMysqlTests(undefined, parsedSubset).catch((error) => {
    console.error('MySQL test runner error:', error);
    process.exit(1);
  });
}
