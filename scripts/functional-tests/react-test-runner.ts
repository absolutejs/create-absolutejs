...
import { computeManifestHash, getOrInstallDependencies, hasCachedDependencies } from './dependency-cache';
...
const buildScaffoldCommand = (
  ...
  const command = ['bun', 'run', 'src/index.ts', projectName, '--skip', '--react'];
...
const validateProject = async (
  ...
  const validationResult = await validateReactFramework(
    projectPath,
    'bun',
    {
      authProvider: config.authProvider,
...
const scaffoldAndTestReact = async (
  config: TestMatrixEntry
) => {
...
const runSequentially = async (
  ...
  handler: (config: TestMatrixEntry, index: number) => Promise<ReactTestResult>
) =>
  configs.reduce<Promise<ReactTestResult[]>>(
...
const printSummary = (results: ReactTestResult[]) => {
  ...
  failedResults.forEach((result) => {
    ...
    console.log(
      `\n- React + ${failureConfig.databaseEngine} + ${failureConfig.orm} + ${failureConfig.authProvider}`
    );
...
export const runReactTests = async (
  ...
  console.log(
    `Testing ${configsToTest.length} React configurations (${matrixEntries.length} total in matrix)...\n`
  );
  ...
  const results = await runSequentially(configsToTest, async (config, index) => {
    ...
    console.log(
      `[${index + 1}/${configsToTest.length}] Testing React + ${config.databaseEngine} + ${config.orm} + ${authLabel}...`
    );

    const outcome = await scaffoldAndTestReact(config);
...
if (import.meta.main) {
  ...
  runReactTests(undefined, parsedSubset).catch((error) => {
    console.error('React test runner error:', error);
    process.exit(1);
  });
}
