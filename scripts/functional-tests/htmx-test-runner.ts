/*
  HTMX Test Runner
  Tests HTMX framework across all compatible backend combinations.
  Uses the test matrix to generate valid HTMX + backend combinations.
*/
...
async function scaffoldAndTestHTMX(
  ...
  const projectName = `test-htmx-${config.databaseEngine}-${config.orm}-${config.authProvider === 'none' ? 'noauth' : 'auth'}-${config.useTailwind ? 'tw' : 'notw'}`.replace(/[^a-z0-9-]/g, '-');
  ...
  try {
    ...
    // Run HTMX validation (skip dependency test since we just installed)
    process.stdout.write('  → Running validation tests... ');
    const validateStart = Date.now();
    const validationResult = await validateHTMXFramework(projectPath, 'bun', {
      databaseEngine: config.databaseEngine,
      orm: config.orm,
      authProvider: config.authProvider,
      useTailwind: config.useTailwind,
...
async function runHTMXTests(
  ...
  console.log(`Testing ${configsToTest.length} HTMX configurations (${htmxConfigs.length} total in matrix)...\n`);

  const results: HTMXTestResult[] = [];
  let passed = 0;
  let failed = 0;
  ...
  for (let i = 0; i < configsToTest.length; i++) {
    ...
    console.log(`[${i + 1}/${configsToTest.length}] Testing HTMX + ${config.databaseEngine} + ${config.orm} + ${config.authProvider === 'none' ? 'no auth' : 'auth'}...`);
    
    const result = await scaffoldAndTestHTMX(config);
    results.push(result);
  ...
  if (failed > 0) {
    ...
    results
      .filter((r) => !r.passed)
      .forEach((r) => {
        console.log(`\n- HTMX + ${r.config.databaseEngine} + ${r.config.orm} + ${r.config.authProvider}`);
...
if (require.main === module) {
  ...
  runHTMXTests(matrixFile, maxConcurrent, testSubset).catch((e) => {
    console.error('HTMX test runner error:', e);
    process.exit(1);
  });
}
