...
async function scaffoldAndTestPostgreSQL(
  ...
  const projectName = `test-postgresql-${config.frontend}-${config.orm}-${config.authProvider === 'none' ? 'noauth' : 'auth'}-${config.databaseHost === 'none' ? 'local' : config.databaseHost}-${config.useTailwind ? 'tw' : 'notw'}`.replace(/[^a-z0-9-]/g, '-');
  ...
  try {
    ...
    // Run PostgreSQL-specific validation
...
async function runPostgreSQLTests(
  ...
  // Filter for PostgreSQL-only configurations (exclude PlanetScale - it's MySQL-only)
  const postgresqlConfigs = matrix.filter(
    (entry) => entry.databaseEngine === 'postgresql' && entry.databaseHost !== 'planetscale'
  );
  
  // Limit to subset if specified
  const configsToTest = testSubset ? postgresqlConfigs.slice(0, testSubset) : postgresqlConfigs;
  
  console.log(`Testing ${configsToTest.length} PostgreSQL configurations (${postgresqlConfigs.length} total in matrix)...\n`);

  const results: PostgreSQLTestResult[] = [];
  let passed = 0;
  let failed = 0;
  ...
  for (let i = 0; i < configsToTest.length; i++) {
    ...
    console.log(`[${i + 1}/${configsToTest.length}] Testing PostgreSQL + ${config.orm} + ${config.authProvider === 'none' ? 'no auth' : 'auth'} + ${config.databaseHost === 'none' ? 'local' : config.databaseHost}...`);
    ...
    const result = await scaffoldAndTestPostgreSQL(config);
    results.push(result);
  ...
  if (failed > 0) {
    ...
    results
      .filter((r) => !r.passed)
      .forEach((r) => {
        console.log(`\n- PostgreSQL + ${r.config.orm} + ${r.config.authProvider} + ${r.config.databaseHost}`);
...
if (require.main === module) {
  ...
  runPostgreSQLTests(matrixFile, maxConcurrent, testSubset).catch((e) => {
    console.error('PostgreSQL test runner error:', e);
    process.exit(1);
  });
}
