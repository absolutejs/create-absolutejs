/*
  SQLite Test Runner
  Tests SQLite database across all compatible backend combinations.
  Uses the test matrix to generate valid SQLite + backend combinations.
*/
...
const validateDatabase = async (
  ...
  process.stdout.write('  → Running SQLite validation... ');

  const validateStartMs = Date.now();
  const validationResult = await validateSQLiteDatabase(projectPath, {
    authProvider: config.authProvider,
    databaseHost: config.databaseHost,
    orm: config.orm
  });
...
const scaffoldAndTestSqlite = async (
  config: TestMatrixEntry
) => {
...
const runSequentially = async (
  ...
  handler: (config: TestMatrixEntry, index: number) => Promise<SqliteTestResult>
) =>
  configs.reduce<Promise<SqliteTestResult[]>>(
...
const printSummary = (results: SqliteTestResult[]) => {
  ...
  failedResults.forEach((result) => {
    ...
    console.log(
      `\n- SQLite + ${failureConfig.databaseHost} + ${failureConfig.orm} + ${failureConfig.authProvider}`
    );
...
export const runSqliteTests = async (
  ...
  console.log(
    `Testing ${configsToTest.length} SQLite configurations (${matrixEntries.length} total in matrix)...\n`
  );
  ...
  const results = await runSequentially(configsToTest, async (config, index) => {
    ...
    console.log(
      `[${index + 1}/${configsToTest.length}] Testing SQLite + ${config.orm} + ${authLabel} + ${hostLabel}...`
    );

    const outcome = await scaffoldAndTestSqlite(config);
...
if (import.meta.main) {
  ...
  runSqliteTests(undefined, parsedSubset).catch((error) => {
    console.error('SQLite test runner error:', error);
    process.exit(1);
  });
}
