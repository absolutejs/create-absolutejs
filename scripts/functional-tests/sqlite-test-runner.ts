...
async function scaffoldAndTestSQLite(
  ...
  try {
    ...
    // Run SQLite-specific validation
...
async function runSQLiteTests(
  ...
  // Filter for SQLite-only configurations
  const sqliteConfigs = matrix.filter((entry) => entry.databaseEngine === 'sqlite');
  
  // Limit to subset if specified
  const configsToTest = testSubset ? sqliteConfigs.slice(0, testSubset) : sqliteConfigs;
  
  console.log(`Testing ${configsToTest.length} SQLite configurations (${sqliteConfigs.length} total in matrix)...\n`);
  ...
  for (let i = 0; i < configsToTest.length; i++) {
    ...
    console.log(`[${i + 1}/${configsToTest.length}] Testing SQLite + ${config.orm} + ${config.authProvider === 'none' ? 'no auth' : 'auth'} + ${config.databaseHost === 'none' ? 'local' : config.databaseHost}...`);
    
    const result = await scaffoldAndTestSQLite(config);
    results.push(result);
  ...
  if (failed > 0) {
    ...
    results
      .filter((r) => !r.passed)
      .forEach((r) => {
        console.log(`\n- SQLite + ${r.config.orm} + ${r.config.authProvider} + ${r.config.databaseHost}`);
...
if (require.main === module) {
  ...
  runSQLiteTests(matrixFile, maxConcurrent, testSubset).catch((e) => {
    console.error('SQLite test runner error:', e);
    process.exit(1);
  });
}
