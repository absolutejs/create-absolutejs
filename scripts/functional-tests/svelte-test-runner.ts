...
async function scaffoldAndTestSvelte(
  ...
  try {
    ...
    // Run Svelte validation (skip dependency test since we just installed)
    process.stdout.write('  → Running validation tests... ');
    const validateStart = Date.now();
    const validationResult = await validateSvelteFramework(projectPath, 'bun', {
      databaseEngine: config.databaseEngine,
      orm: config.orm,
      authProvider: config.authProvider,
      useTailwind: config.useTailwind,
...
async function runSvelteTests(
  ...
  console.log(`Testing ${configsToTest.length} Svelte configurations (${svelteConfigs.length} total in matrix)...\n`);

  const results: SvelteTestResult[] = [];
  let passed = 0;
  let failed = 0;
  ...
  for (let i = 0; i < configsToTest.length; i++) {
    ...
    console.log(`[${i + 1}/${configsToTest.length}] Testing Svelte + ${config.databaseEngine} + ${config.orm} + ${config.authProvider === 'none' ? 'no auth' : 'auth'}...`);
    
    const result = await scaffoldAndTestSvelte(config);
    results.push(result);
  ...
  if (failed > 0) {
    ...
    results
      .filter((r) => !r.passed)
      .forEach((r) => {
        console.log(`\n- Svelte + ${r.config.databaseEngine} + ${r.config.orm} + ${r.config.authProvider}`);
...
if (require.main === module) {
  ...
  runSvelteTests(matrixFile, maxConcurrent, testSubset).catch((e) => {
    console.error('Svelte test runner error:', e);
    process.exit(1);
  });
}
