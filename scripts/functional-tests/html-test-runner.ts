...
const buildScaffoldCommand = (
  ...
  const command = ['bun', 'run', 'src/index.ts', projectName, '--skip', '--html'];
...
const validateProject = async (
  ...
  const validationResult = await validateHTMLFramework(
    projectPath,
    'bun',
    {
      authProvider: config.authProvider,
...
const scaffoldAndTestHtml = async (
  config: TestMatrixEntry
) => {
...
const runSequentially = async (
  ...
  handler: (config: TestMatrixEntry, index: number) => Promise<HtmlTestResult>
) =>
  configs.reduce<Promise<HtmlTestResult[]>>(
...
const printSummary = (results: HtmlTestResult[]) => {
  ...
  failedResults.forEach((result) => {
    ...
    console.log(
      `\n- HTML + ${failureConfig.databaseEngine} + ${failureConfig.orm} + ${failureConfig.authProvider}`
    );
...
export const runHtmlTests = async (
  ...
  console.log(
    `Testing ${configsToTest.length} HTML configurations (${matrixEntries.length} total in matrix)...\n`
  );
  ...
  const results = await runSequentially(configsToTest, async (config, index) => {
    ...
    console.log(
      `[${index + 1}/${configsToTest.length}] Testing HTML + ${config.databaseEngine} + ${config.orm} + ${authLabel}...`
    );

    const outcome = await scaffoldAndTestHtml(config);
...
if (import.meta.main) {
  ...
  runHtmlTests(undefined, parsedSubset).catch((error) => {
    console.error('HTML test runner error:', error);
    process.exit(1);
  });
}
