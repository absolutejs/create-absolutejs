...
async function scaffoldAndTestMongoDB(
  ...
  const projectName = `test-mongodb-${config.frontend}-${config.orm}-${config.authProvider === 'none' ? 'noauth' : 'auth'}-${config.databaseHost === 'none' ? 'local' : config.databaseHost}-${config.useTailwind ? 'tw' : 'notw'}`.replace(/[^a-z0-9-]/g, '-');
  ...
  try {
    ...
    // Note: MongoDB may not fully support Drizzle, but we'll test what's in the matrix
    ...
    // Run MongoDB-specific validation
...
async function runMongoDBTests(
  ...
  // Filter for MongoDB-only configurations
  const mongodbConfigs = matrix.filter((entry) => entry.databaseEngine === 'mongodb');
  
  // Limit to subset if specified
  const configsToTest = testSubset ? mongodbConfigs.slice(0, testSubset) : mongodbConfigs;
  
  console.log(`Testing ${configsToTest.length} MongoDB configurations (${mongodbConfigs.length} total in matrix)...\n`);

  const results: MongoDBTestResult[] = [];
  let passed = 0;
  let failed = 0;
  ...
  for (let i = 0; i < configsToTest.length; i++) {
    ...
    console.log(`[${i + 1}/${configsToTest.length}] Testing MongoDB + ${config.orm} + ${config.authProvider === 'none' ? 'no auth' : 'auth'} + ${config.databaseHost === 'none' ? 'local' : config.databaseHost}...`);
    ...
    const result = await scaffoldAndTestMongoDB(config);
    results.push(result);
  ...
  if (failed > 0) {
    ...
    results
      .filter((r) => !r.passed)
      .forEach((r) => {
        console.log(`\n- MongoDB + ${r.config.orm} + ${r.config.authProvider} + ${r.config.databaseHost}`);
...
if (require.main === module) {
  ...
  runMongoDBTests(matrixFile, maxConcurrent, testSubset).catch((e) => {
    console.error('MongoDB test runner error:', e);
    process.exit(1);
  });
}
