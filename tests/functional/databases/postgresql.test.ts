...
import { validatePostgreSQLDatabase } from '../../../scripts/functional-tests/postgresql-validator';
import { runMatrixSuite } from '../frameworks/test-utils';
import { ensureDockerAvailable } from '../support';

type PostgresMatrixEntry = MatrixConfig & {
...
const createProjectName = (config: PostgresMatrixEntry) =>
...
const describeConfig = (config: PostgresMatrixEntry) => {
  const segments = [
    'PostgreSQL',
    config.databaseHost === 'none' ? 'local' : config.databaseHost,
...
runMatrixSuite({
  createProjectName, describeBlock: 'PostgreSQL database matrix', describeConfig, beforeValidate: async ({ config, projectPath }) => {
    ...
    orm: config.orm,
    useTailwind: config.useTailwind
  }), filterMatrix: (config): config is PostgresMatrixEntry =>
    ...
    const { errors, passed, warnings } = await validatePostgreSQLDatabase(projectPath, {
      authProvider: config.authProvider,
      databaseHost: config.databaseHost,
      orm: config.orm
    });
